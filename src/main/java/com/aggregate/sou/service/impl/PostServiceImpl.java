...
import com.aggregate.sou.mapper.PostFavourMapper;
import com.aggregate.sou.mapper.PostMapper;
import com.aggregate.sou.mapper.PostThumbMapper;
import com.aggregate.sou.model.dto.post.PostEsDTO;
import com.aggregate.sou.model.dto.post.PostQueryRequest;
import com.aggregate.sou.model.entity.Post;
import com.aggregate.sou.model.entity.PostFavour;
import com.aggregate.sou.model.entity.PostThumb;
import com.aggregate.sou.model.entity.User;
...
import com.aggregate.sou.utils.SqlUtils;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.google.gson.Gson;
import com.aggregate.sou.constant.CommonConstant;
...
public class PostServiceImpl extends ServiceImpl<PostMapper, Post> implements PostService {
    ...
    public Page<Post> searchFromEs(PostQueryRequest postQueryRequest) {
        ...
        NativeSearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
                .withPageable(pageRequest).withSorts(sortBuilder).withHighlightFields(contentField, titleField).build();
        SearchHits<PostEsDTO> searchHits = elasticsearchRestTemplate.search(searchQuery, PostEsDTO.class);
        Page<Post> page = new Page<>();
        page.setTotal(searchHits.getTotalHits());
        List<Post> resourceList = new ArrayList<>();

        List<PostEsDTO> searchAnswerList = new ArrayList();
        for (SearchHit<PostEsDTO> searchHit : searchHits) { // 获取搜索到的数据
            PostEsDTO contentHit = searchHit.getContent();
            // 处理高亮
            ...
            for (Map.Entry<String, List<String>> stringHighlightFieldEntry : highlightFields.entrySet()) {
                ...
                if (StringUtils.equals(key, "content")) {
                    ...
                    contentHit.setContent(sb.toString());
                }
                if (StringUtils.equals(key, "title")) {
                    ...
                    contentHit.setTitle(sb.toString());
                }
            }
            searchAnswerList.add(contentHit);
        }
...
/*        List<Post> postList = new ArrayList<>();
        for (PostEsDTO postEsDTO : searchAnswerList) {
            Post post = new Post();
            post.setId(postEsDTO.getId());
            post.setTitle(postEsDTO.getTitle());
            post.setContent(postEsDTO.getContent());
            //post.setTags("");
            post.setFavourNum(0);
            post.setThumbNum(0);
            post.setUserId(postEsDTO.getUserId());
            post.setCreateTime(postEsDTO.getCreateTime());
            post.setUpdateTime(postEsDTO.getUpdateTime());
            post.setIsDelete(postEsDTO.getIsDelete());
            postList.add(post);
        }*/
